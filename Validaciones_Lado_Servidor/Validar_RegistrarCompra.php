    ...
    function main() {
        ...
        if($this->errores){
            ...
            if($objEntrada->InsertarEntrada($id_Entrada, $Fecha, $referencia, $tipo_movimiento, $entrada_estatus)){
                ...
                $detalles_entrada =  $objDetalleEntrada->RegistrarDetalleEntrada($id_Entrada, $Compras, 1);
                if(count($detalles_entrada)>0)
                {
                    ...
                    $objEntrCompras = new EntradaCompras();
                    $objEntrCompras->Insertar($id_Entrada, $NumDocumento, $IdProveedor, $Observaciones, $CostoTotal, $IdEncargado);
...
$objRegistrarCompra = new RegistrarCompra();
