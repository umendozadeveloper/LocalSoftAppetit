...
class AgregarConcepto{
    ...
    function main(){
        ...
        if($this->errores){
            ...
            if($this->objConcepto->Insertar($this->objConcepto->Clave, $this->objConcepto->Descripcion,
                    $this->objConcepto->ES,  $this->objConcepto->Observaciones,  $this->objConcepto->Estatus)){
                $_SESSION['valClaveConcepto'] = NULL;
                $_SESSION['valDescrConcepto'] = NULL;
                $_SESSION['valES']=NULL;
                $_SESSION['valObservac'] = null;
                $_SESSION['valEstatus'] = null;
                setSuccessMessage("Concepto agregado correctamente");
                header("Location: ../F_A_DetalleConcepto.php?IdConcepto=".$this->objConcepto->ID);
            }  
            else{
                setSwalFailureMessage("Ocurrió un error, el concepto no se pudo registrar. Intente más tarde.");
                header("Location: ../F_A_RegistrarConceptos.php");
